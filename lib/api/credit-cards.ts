...
export async function createCreditCard(
  creditCard: Omit<CreditCard, "id" | "created_at" | "updated_at">,
): Promise<CreditCard> {
  try {
    const { data, error } = await supabase.from("credit_cards").insert([creditCard]).select().single()
...
export async function getCreditCardSummary(userId: string) {
  try {
    ...
    const summary = {
      ...
      totalAvailableLimit: activeCards.reduce(
        (sum, card) => sum + ((card.credit_limit || 0) - (card.current_debt || 0)),
        0,
      ),
