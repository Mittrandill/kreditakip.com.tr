...
// Simple PDF Report Modal for basic usage
interface SimplePDFReportModalProps {
  open: boolean
...
export function PdfReportModal({ open, onOpenChange, credits, paymentHistory, metrics }: SimplePDFReportModalProps) {
  const [isGenerating, setIsGenerating] = useState(false)
  const { toast } = useToast()
    ...
    try {
      const reportData = {
        ...
        credits: credits.map((credit) => ({
          ...
          creditType: credit.credit_type || "Diğer",
          remainingDebt: credit.remaining_amount || 0,
          monthlyPayment: credit.monthly_payment || 0,
          interestRate: credit.interest_rate || 0,
          status: credit.status || "unknown",
          amount: credit.original_amount || 0,
        })),
        payments: paymentHistory.map((payment) => ({
          id: payment.id,
          date: payment.payment_date,
          bankName: payment.bank_name || "Bilinmeyen",
          amount: payment.amount || 0,
      ...
      toast({
        title: "✅ Rapor Hazırlandı",
        description: "PDF raporunuz başarıyla oluşturuldu ve indirildi.",
      })
      ...
      toast({
        variant: "destructive",
        title: "❌ Hata Oluştu",
        description: "Rapor oluşturulurken bir sorun yaşandı.",
      })
    } finally {
      setIsGenerating(false)
    }
        ...
        <div className="space-y-4">
          <div className="text-center">
            <p className="text-gray-600">
              {credits.length} kredi ve {paymentHistory.length} ödeme kaydı ile rapor oluşturulacak.
            </p>
          </div>
